...
public class NpcSpawner : MonoBehaviour
{
     ...
     int npcMaxNumber = GameManager.level * 20;
     int npcNumber = 0;
    ...
    public static float spawnTime = 18 - GameManager.level;
    ...
    private void Update()
    {
     ...
     maxProbabilityRange = GameManager.level * GameManager.difficultyStep;
        ...
        // with each level the spawn time decrease
     spawnTime = Random.Range(12 - GameManager.level, 15 - GameManager.level) * spawnTimeMultiplier;
    }
    ...
    IEnumerator spawnAnNPC()
    {
        ...
        if (npcNumber < npcMaxNumber)
        {
         ...
         //  print("npcNumber: " + npcNumber + "npcMaxNumber: " + npcMaxNumber);
            ...
            npc.GetComponent<NpcController>().moveSpeed = Random.Range(GameManager.level, GameManager.level + 3);
            npc.GetComponent<NpcController>().moveTime = Random.Range(GameManager.level, GameManager.level + 7);
            npc.GetComponent<NpcController>().idleTime = Random.Range(GameManager.level, GameManager.level + 2);


            npc.GetComponent<NPC>().coolDownTime = Random.Range(8 - GameManager.level, 12 - GameManager.level);
